...
enum Modes {NUMERIC = 0, ALPHANUMERIC, BYTE};
...
typedef struct {
    int penalty[4];
    unsigned char mask;
    unsigned char error_MaskPattern[5];
...
typedef struct {
    //char *name;
    char version; // de 1 a 3
    char correction; // L, M, Q, H
    Text fraseInfo;
    CodeWords cw;
    //Matrix *m;
    char show;
...
void BestMask_FormatInformation(QRInfo *qr);

void saveImage(Image i);

void Teste_printTableUC(unsigned char *v, int tam) { //teste
    int i;
    for(i=0; i<tam; i++) {
        printf("%i ", v[i]);
        if((i+1)%5==0)
...
void QRCodeMain(char *text, char error) {
    QRInfo *qr;
    ...
    /*Dark Module*/
    *M_at(qr->image.m, _1, 4*(qr->version+1)+9, 8)= BLACK;

    AddReservedAreas(qr);
    PlaceDataBits(qr);

    BestMask_FormatInformation(qr); // ...
...
void readFileInt(char *name, int v[], int tam) {
    FILE *fp;
    if((fp = fopen(name, "rb"))!= NULL) {
        fread(v, tam, sizeof(int), fp);
        fclose(fp);
    } else
        fprintf(stdout, "erro ao abrir o arquivo");
}
...
void AlphanumericEncode(QRInfo *qr) {
    ...
    M_fill(aux, qr->fraseInfo.text, qr->fraseInfo.text+qr->fraseInfo.tamanho);
//    LOGS(qr->show, printf("Letras aos pares:");)
//    M_print(aux);
...
void ByteEncode(QRInfo *qr) {
    ...
    for(i=0; i< qr->fraseInfo.tamanho; i++) {
        ...
        printf("%c - %x - %i - %s\n", num, num, num, bits); //o num vai dividindo até chegar a 0
        strcat(qr->fraseInfo.bits.modeEncode.elements, bits);
    }
    LOGS(qr->show, printf("\n\t-------------------\n\n");)
}

void Teste_writeFile(char *name, char *v, int tam);
int toDecimal(int base, char bin[]);
void setCodigosCorretores(QRInfo *qr);
void SetCodeWords(QRInfo *qr) { // TABELA DOS BLOCOS
    LOGS(qr->show, printf("\t----Set Code Words---\n");)
    char v[36];
    readFileChar("ErrorCorrectionTable.bin", v, 36);

    /*organiza a tabela e recolhe os dados*/
    Matrix *tabela;
...
//    MSG("strFinal organizada de 8 em 8:")
//    M_print(strBits);
    char number[9];
    int enchimento[]= {236, 17}, zero = 0; // 0 e 1
    //MSG("Palavras codigo ja com enchimento 236 e 17, mas sem printar ai embaixo:")
    for(index=0; index<qr->cw.palavrasCodigo.tamBits; index++) {
        if(index<grupos_8) {
            //printf("*M_at = %i\n", *M_at(strBits, _1, index, 0));
            for(i=0; i<8; i++) {
                if(index*strBits->C + i < qr->fraseInfo.bits.strFinal.tamBits)
                    number[i] = *M_at(strBits, _1, index, i); // '0' ou '1'
                else
                    number[i] = '0';
            }
            number[i]=0;
            ...
            //printf("Palavra codigo %i:  number bin da strFinal: %s - dec: %i\n", index, number, qr->cw.palavrasCodigo.elements[index]);
        } else {
...
void setCodigosCorretores(QRInfo *qr) {
    int i, index=0, bloco;
    qr->cw.codigosCorretores.elements = (unsigned char*)calloc(qr->cw.codigosCorretores.tamBits, sizeof(char));
...
void StructureFinalMessage(QRInfo *qr) {
    /*reorganizando as PC e CC continuarão com o mesmo tamanho, só bagunçadas*/
    ...
    /*organizando as palavras codigo*/
    Matrix *pc;
    pc = M_create(1, (unsigned int)qr->cw.blocks, (unsigned int)qr->cw.pcPerBlock); //numero de pc por bloco;
    ...
    /*organizando os codigos corretores*/
    i=0;
    Matrix *cc; // codigos corretores
    cc = M_create(1, (unsigned int)qr->cw.blocks, (unsigned int)qr->cw.ccPerBlock); //numero de pc por bloco;
...
void finalMsgToBits(QRInfo *qr) {
    ...
    // a tabela para bits restantes até a versão 3 é monotona
    ...
    int num; //tem que mandar do tipo char porque o strlen não dá pra usar com ponteiro!
    //pois ponteiro é só uma variável
    for(i=0; i<qr->cw.finalMessage.tamBits; i++) { //qr->cw.finalMessage.tamBits são BYTES
...
/*Agora comeca a parte de construir a imagem*/
...
void drawFinderPattern(QRInfo *qr, int x, int y) {
    ...
    //(y+2 && y+2+lado-1) x+2----direita
    *M_at(qr->image.m, _1, x+3, y+3) = BLACK; // para os quadrados linhas e colunas sao a mesma coisa
...
void AddAlignmentPatterns(QRInfo *qr) {
    //TABELA, mas é simples para versao 2 e 3
    int coord[2],i, j;
    switch(qr->version) {
        case _2: // numero da versao
            coord[0]=6;
            coord[1]=18;
            break;
        case _3:
...
void drawSquare(QRInfo *qr, int y, int x, int lado, unsigned char MODULE) { // linha e coluna
    int i;
    for(i=0; i<lado; i++) {
        //if(*M_at(qr->image.m, _1, x+i, y)!=BLACK && *M_at(qr->image.m, _1, x+i, y)!=WHITE) //
...
void AddReservedAreas(QRInfo *qr) {
    ...
    /*Version Information Area - só para QR codes com versao >= 7*/
//    int index;
//    for(index=0; index<3; index++) {
//        i.row=0;
//        i.col=qr->image.m->C-9 - index;
//        f.row=5;
//        f.col=qr->image.m->C-9 - index;
//        drawLine(qr, i, f, 'V');
//
//        i.row= f.row= qr->image.m->R-9 - index;
//        i.col=0;
//        f.col=5;
//        drawLine(qr, i, f, 'V');
//    }
...
/* Data Mask and format information */

void chooseLowestPenalty(QRInfo *qr);
void evaluationCondition1(QRInfo *qr);
void evaluationCondition4(QRInfo *qr);
void FormatInformation(QRInfo *qr);
void BestMask_FormatInformation(QRInfo *qr){ // penalty = multa
    FormatInformation(qr);
    evaluationCondition1(qr);
//    evaluationCondition2(qr); // ...
//    evaluationCondition3(qr);
    evaluationCondition4(qr);

    chooseLowestPenalty(qr);
}
void FormatInformation(QRInfo *qr){
...
/*salva a imagem mas está estranho - só para o CLion*/
...
void saveImage(Image image) {
    ...
    if((fp = fopen(image.name, "w"))!= NULL) {
        fprintf(fp, "P%d\n%d %d\n", image.type, image.m->R, image.m->C);
        for(i=0; i<image.m->R; i++) {
            for(j=0; j<image.m->C; j++)
                fprintf(fp, "%d ", !!(int)(*M_at(image.m, _1, i, j) - '0'));
            fprintf(fp, "\n");
        }
        fclose(fp);
    } else
